...
server.post('/api/users', (req, res) => {
  ...
  users.insert(req.body)
    ...
    .catch( () => {
      return res.status(500).json({ message: "There was an error while saving the user" })
    })
})
...
server.get('/api/users', (req, res) => {
  users.find()
    ...
    .catch( () => {
      return res.status(500).json({ message: "The users could not be retrieved" })
    })
})
...
server.get('/api/users/:id', (req, res) => {
  users.findById(req.params.id)
    .then( user => {
      if(user === undefined)
        return res.status(404).json({ message: `The user with Id ${req.params.id} does not exist` })
        // return res.status(404).json({message: `Could not find user with id ${req.params.id}`})
      return res.status(200).json(user)
    })
    .catch( () => {
      return res.status(500).json({ message: "The user could not be retrieved" })
    })
})
